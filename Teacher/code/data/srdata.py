...
class SRData(data.Dataset):
        ...
        if args.ext == 'img' or benchmark:
            ...
            if args.ext.find('reset') >= 0:
                ...
                for v in self.images_hr:
                    hr = imageio.imread(v, as_gray= self.as_gray)
                    if self.as_gray == True:
                        hr = np.expand_dims(hr, axis=2)
                    name_sep = v.replace(self.ext, '.npy')
                    np.save(name_sep, hr)
                for si, s in enumerate(self.scale):
                    for v in self.images_lr[si]:
                        lr = imageio.imread(v, as_gray= self.as_gray)
                        if self.as_gray == True:
            ...
            self.images_lr = [[v.replace(self.ext, '.npy') for v in self.images_lr[i]]for i in range(len(self.scale))]
            ...
            try:
                ...
                hr = []
                for f in list_hr:
                    hr_ = imageio.imread(f, as_gray= self.as_gray)
                    if self.as_gray == True:
                        hr_ = np.expand_dims(hr_, axis=2)
                    hr.append(hr_)
                ...
                for si, s in enumerate(self.scale):
                    lr_scale = []
                    for f in list_lr[si]:
                        lr_ = imageio.imread(f, as_gray=self.as_gray)
                        if self.as_gray == True:
                            lr_ = np.expand_dims(lr_, axis=2)
                        lr_scale.append(lr_)
    ...
    def _load_file(self, idx):
        ...
        if self.args.ext == 'img' or self.benchmark:
            filename = hr
            lr = imageio.imread(lr, as_gray= self.as_gray)
            if self.as_gray == True:
                lr = np.expand_dims(lr, axis=2)
            hr = imageio.imread(hr, as_gray= self.as_gray)
            if self.as_gray == True:
                hr = np.expand_dims(hr, axis=2)
